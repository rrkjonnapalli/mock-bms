...
class BookingMethods extends BaseMethods {
  ...
  static async checkFreeSeats(show) {
    let { ok, error, result } = await handlePromise(mongoose.model(MODEL_NAMES.SHOW)
      .findById(show)
      .select('totalSeats freeSeats').lean());
    if (!ok) { throw error; }
    if (!result) { throw new Error('Show not found'); }
    const { totalSeats, freeSeats } = result;
    if (freeSeats <= 0) { return []; }
    const pipeline = [
      { $match: { show: new mongoose.Types.ObjectId(show) } },
      { $group: { _id: '$show', booked: { $addToSet: '$seat' } } }
    ];
    ({ ok, error, result } = await handlePromise(mongoose.model(MODEL_NAME).aggregate(pipeline)));
    if (!ok) { throw error; }
    if (result.length === 0) {
      return Array.from({ length: totalSeats }, (_, i) => i + 1);
    }
    const { booked: bookedSeats } = result[0];
